...
// text message build
  ...
  function bldmsg($rs, $dtl, $rsUser) {
    ...
    $customer_notes = mysql_result($rs,0,'customernotes');
    $html_codes =     array("&#038;", "&#034;", "&#039;", "&#037;", "&#040;", "&#041;", "&#010;", "&#010;", "&#010;");
    $special_chars =  array("&",      "\"",     "'",      "%",      "(",      ")",      "\r\n",   "\r",     "\n");
    ...
    $msg .= "Name:\t\t\t".mysql_result($rsUser,0,'firstName')." ".mysql_result($rsUser,0,'lastName')."\n\n";
    $msg .= "Company:\t\t".mysql_result($rsUser,0,'company')."\n\n";
    $msg .= "Phone:\t\t".mysql_result($rsUser,0,'phone1')." OR ".mysql_result($rsUser,0,'phone2')."\n\n";
    $msg .= "Account:\t\t".mysql_result($rsUser,0,'AcctName')."\n\n";
    $msg .= "Ship To:\t\t".mysql_result($rs,0,'shipcompany')."\n";
    $msg .= "\t\t\t".mysql_result($rs,0,'shipaddress')."\n";
    $msg .= "\t\t\t".mysql_result($rs,0,'shipcity').', '.mysql_result($rs,0,'shipprov').' '.mysql_result($rs,0,'shipzip')."\n\n";

    if (strlen(trim(mysql_result($rs, 0, 'documentname'))) > 0)
    ...
    $msg .= "Your order will be ready by: ".mysql_result($rs,0,'readydate').".\n";
    $msg .= "Order Number: ".mysql_result($rs,0,'ID')."\n";
    $msg .= "Your Reference Info: ".mysql_result($rs,0,'refnum')."\n\n";
    ...
    $discount_dollars = mysql_result($rs,0,'discount');
    ...
    $totsetup = substr(mysql_result($rs,0,'setupfee'), 1);
    $totsetup = getcurrency($totsetup);
    $msg .= str_pad("Setup:",36).$totsetup."\n";

    $thepromocode = mysql_result($rs,0,'promocode');
    $thepromodiscount = mysql_result($rs,0,'promodiscount');
    ...
    $therushfee = substr(mysql_result($rs,0,'rushfee'), 1);
    if ($therushfee != 0) {
    ...
    if (strncmp(mysql_result($rs,0,'freight'), '$', 1) == 0)
      $thefreight = substr(mysql_result($rs,0,'freight'), 1); // strip off the $ sign
    else
      $thefreight = substr(mysql_result($rs,0,'freight'), 0); // pass through the whole value

    $thefreight = getcurrency($thefreight);
    $msg .= str_pad("Freight:",10).str_pad(mysql_result($rs,0,'shiptype'),26).getcurrency($thefreight)."\n";

    $thepst = substr(mysql_result($rs,0,'pst'), 1);
    if ($thepst != 0) {
      $thepst = getcurrency($thepst);
      $msg .= str_pad("PST:",36).getcurrency($thepst)."\n";
    }
    $thegst = substr(mysql_result($rs,0,'gst'), 1);
    if ($thegst != 0) {
      $thegst = getcurrency($thegst);
      $msg .= str_pad("GST:",36).getcurrency($thegst)."\n";
    }

    $msg .= str_pad("Total:",36).mysql_result($rs,0,'ordertotal')."\n\n";
