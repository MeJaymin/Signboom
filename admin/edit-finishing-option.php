<?php 
  ...
  $edited = false;
  $edit_mode = "edit";
  if (!isset($_REQUEST['finishing_option_id']))
  {
    echo "You must specify a finishing option id. Please visit the <a href=\"select-finishing-option.php\">Finishing Options</a> page and click an option there.";
  }
  else
  {
    $finishing_option_id = $_REQUEST['finishing_option_id'];
    if (!isValidFinishingOptionId($finishing_option_id)) // same validation code as for product id
    {
      echo "'$finishing_option_id' is not a valid finishing option id. Please visit the <a href=\"select-finishing-option.php\">Finishing Options</a> page and click a finishing option.";
    }
    else 
    {
      if (isset($_REQUEST['submit_edit_finishing_option'])) 
      {
        ...
        if (strlen($product_category) == 0)
          ...
          $error_message = "'$finishing_option_name' is not a finishing option name. Only letters, numbers, dashes, periods, spaces and () are allowed. The name must be between 4 and 64 characters long.";
        else if (strlen($finishing_short_name) > 8) 
          $error_message = "'$finishing_short_name' is too long. Short Name MUST be 8 characters or less.";
...
*/
        ...
        {
          $query = "UPDATE signboom_finishing SET Category = '$product_category', BatchDay = $finishing_option_batch_day, OptionSet = '$finishing_option_set', OptionType = '$finishing_option_type', OptionName = '$finishing_option_name', ShortName = '$finishing_short_name', Queue = '$finishing_option_queue', Description = '$finishing_option_description',  Code = '$finishing_option_code', ExtraTime= '$extra_time', Fixed = '$finishing_option_fixed_cost', Variable = '$finishing_option_variable_cost', SortGroup = '$finishing_option_sort_group', SortOrder = '$finishing_option_sort_order', Units= '$units_of_measure', UnitsPerHour= '$units_per_hour', Reference = '$reference' WHERE Id = $finishing_option_id";
    ...
    } // valid finishing code id passed in

  } // finishing code id passed in 
