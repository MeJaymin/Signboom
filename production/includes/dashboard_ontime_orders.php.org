...
End_Of_Query;
  ...
  if ($earlylate == 1) { 
    $orders = mysqli_query( $DBConn, $query_orders_early) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
    $numerator = mysqli_num_rows($orders);
    return($numerator);
  }
  // Looking for late jobs.
  else if ($earlylate == 0) { 
    $orders = mysqli_query( $DBConn, $query_orders_late) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
    $numerator = mysqli_num_rows($orders);
    return($numerator);
  }
}
...
End_Of_Query;
  ...
  $orders = mysqli_query( $DBConn, $query_orders) or die("calculateOnTime: Could not read orders from database:" . mysqli_error($GLOBALS["___mysqli_ston"]) . "<br><br>" . $query_orders);
  $denominator = mysqli_num_rows($orders);
