...
End_Of_Query;
  ...
  $completed_jobs = mysqli_query( $DBConn, $query_completed_jobs) or die("queryOrdersComplete: Could not read orders from database:" . mysqli_error($GLOBALS["___mysqli_ston"]) . "<br><br>" . $query_completed_jobs);
  $number_of_completed_jobs = mysqli_num_rows($completed_jobs);
