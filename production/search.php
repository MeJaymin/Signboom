...
if(isset($_POST['find_order']))
{
	...
	if (strlen($order_id) > 0)
	{
		...
		// bring up single order page for that order
		// header("Location: http://10.235.4.47/SignBoom/production/orderitem.php?order_id=" . $order_id);
		header("Location: " .$protocol. "://" . $host . "/Signboom/production/orderitem.php?order_id=" . $order_id);
	}
}
/** Added by zCon **/
